...
## 2026-06-08 - [Behavioral Velocity Profiling & Bot Mitigation]
**Vulnerability:** Standard `isTrusted` checks can be bypassed by sophisticated automation, and basic rate limiting does not account for sub-human interaction speeds.
**Learning:** Measuring the velocity between interactions (e.g., clicks) provides a secondary layer of behavioral verification. Bots often trigger events with perfect timing or at speeds impossible for humans. By persisting interaction timestamps in the global state, we can detect these anomalies.
**Prevention:** Implement a velocity-based interaction guard in `useSentinel` that enforces a minimum delay (e.g., 50ms) between significant UI actions. Couple this with autonomous triggers in `useMoltAutomation` to harden the system upon detection.
