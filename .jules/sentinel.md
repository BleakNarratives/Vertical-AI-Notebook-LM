...
## 2026-07-03 - [Behavioral Velocity Profiling & Secure Randomness]
**Vulnerability:** Deterministic decoy rotation and sub-human interaction speeds (automation) bypassing rate limits.
**Learning:** `Math.random()` is predictable and unsuitable for security decoys. Furthermore, simple rate limits can be bypassed by sophisticated scripts that operate just below the threshold. Behavioral velocity profiling (measuring delta between events) provides a second layer of defense.
**Prevention:** Always use `window.crypto.getRandomValues()` for security-sensitive randomness. Implement velocity profiling in interaction handlers to detect and block sub-human interaction speeds (e.g., < 50ms).
