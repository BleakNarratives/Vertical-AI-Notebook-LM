...
## 2026-06-08 - [Behavioral Velocity Profiling & Synthetic Interaction Detection]
**Vulnerability:** Lack of heuristic verification for interaction speed, allowing high-frequency automated scripts to bypass standard rate limits.
**Learning:** Pure rate limiting (count per window) can be gamed by bots that operate just under the threshold but with sub-human precision and speed. Interaction velocity (ms between events) provides a higher-fidelity signal for synthetic activity.
**Prevention:** Implement velocity tracking on the global window object. Enforce a minimum interaction delta (e.g., 50ms) for critical events like 'click'. Link velocity alerts to autonomous system hardening (Molt) to increase the cost of automated probing.
