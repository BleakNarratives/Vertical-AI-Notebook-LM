    ...
    <div className="absolute top-12 left-4 flex flex-col items-center gap-2">
      ...
      <button
        ...
        className={`group relative z-20 w-32 h-48 bg-grey-dark border transition-all hover:scale-105 focus-visible:scale-105 active:translate-y-1 outline-none transform-gpu cursor-none ${
          isActive ? 'border-neon-amber shadow-[0_0_20px_rgba(255,191,0,0.3)]' : 'border-grey-medium shadow-2xl'
        }`}
      >
        <FocusIndicator color="neon-amber" shortcutKey="W" />
        <div className="absolute inset-2 bg-obsidian/50 border border-grey-dark p-2 overflow-hidden">
          ...
          <svg className={`absolute inset-0 w-full h-full opacity-20 pointer-events-none transition-colors duration-500 ${isActive ? 'stroke-neon-amber' : 'stroke-neon-red'}`} viewBox="0 0 100 150" fill="none">
