  ...
  return (
    <div className="absolute top-16 -right-12 flex flex-col items-center gap-2">
      ...
      <button
        ...
        style={{ transform: 'rotateX(-25deg) rotateZ(var(--tw-rotate, 2deg)) translateY(var(--tw-translate-y, 0)) scale(var(--tw-scale-x, 1), var(--tw-scale-y, 1))' }}
      ...
      >
        ...
        <div className="absolute -inset-2 opacity-0 group-focus-visible:opacity-100 transition-opacity pointer-events-none">
          <div className={`absolute top-0 left-0 w-2 h-2 border-t border-l transition-colors duration-500 ${isActive ? 'border-neon-amber' : 'border-neon-red'}`} />
          <div className={`absolute top-0 right-0 w-2 h-2 border-t border-r transition-colors duration-500 ${isActive ? 'border-neon-amber' : 'border-neon-red'}`} />
          <div className={`absolute bottom-0 left-0 w-2 h-2 border-b border-l transition-colors duration-500 ${isActive ? 'border-neon-amber' : 'border-neon-red'}`} />
          <div className={`absolute bottom-0 right-0 w-2 h-2 border-b border-r transition-colors duration-500 ${isActive ? 'border-neon-amber' : 'border-neon-red'}`} />
        </div>
      </button>
    </div>
  );
