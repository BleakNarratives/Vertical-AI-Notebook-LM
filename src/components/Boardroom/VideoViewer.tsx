  ...
  return (
    <div className="absolute top-16 -right-12 flex flex-col items-center gap-2">
      ...
      <button
        ...
        className={`group/monitor relative z-20 w-40 h-28 bg-grey-dark border border-grey-medium shadow-2xl transition-all hover:scale-105 focus-visible:scale-105 active:translate-y-1 outline-none transform-gpu`}
      >
        ...
        <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-xs font-mono text-neon-amber opacity-0 group-hover/monitor:opacity-100 group-focus-visible/monitor:opacity-100 transition-opacity uppercase whitespace-nowrap">
          Monitor
        </span>

        <FocusIndicator color={isActive ? "neon-amber" : "neon-red"} group="monitor" />
      </button>
    </div>
  );
};
