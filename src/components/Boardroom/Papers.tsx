...
const Paper: React.FC<PaperProps> = ({ label, title, rotation = '0deg', translateY = '0px', isActive, onClick, labelPosition = 'top' }) => {
  return (
    <button
      ...
      className={`group/stack relative w-12 h-16 bg-white/5 border transition-all hover:rotate-0 hover:translate-y-0 hover:scale-110 active:translate-y-1 hover:bg-white/10 shadow-lg outline-none transform-gpu ${isActive ? 'border-neon-amber shadow-[0_0_25px_rgba(255,191,0,0.4)]' : 'border-grey-medium'}`}
    >
      ...
      <span className={`absolute left-1/2 -translate-x-1/2 text-xs font-mono text-neon-amber opacity-0 group-hover/stack:opacity-100 group-focus-visible/stack:opacity-100 transition-opacity uppercase whitespace-nowrap ${labelPosition === 'top' ? '-top-8' : '-bottom-8'}`}>
        {title}
      </span>
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="group/papers relative flex items-end justify-center w-24 h-24 mb-4">
        {/* Paper 1: Bottom Left */}
        <div className="absolute bottom-0 left-2 transition-all duration-300 group-hover/papers:-translate-x-6 group-focus-within/papers:-translate-x-6 group-hover/papers:-rotate-6 group-focus-within/papers:-rotate-6">
          <Paper
            label={`View ${data.p1}`}
            title={data.p1}
            rotation="-2deg"
        ...
        <div className="absolute bottom-0 right-2 transition-all duration-300 group-hover/papers:translate-x-6 group-focus-within/papers:translate-x-6 group-hover/papers:rotate-6 group-focus-within/papers:rotate-6">
          <Paper
            label={`View ${data.p3}`}
            title={data.p3}
            rotation="4deg"
