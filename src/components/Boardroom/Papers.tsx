...
const Paper: React.FC<PaperProps> = ({ label, title, rotation = '0deg', translateY = '0px', isActive, onClick }) => {
  return (
    <button
      ...
      className={`group/paper relative w-12 h-16 bg-white/5 border transition-all hover:rotate-0 hover:translate-y-0 hover:scale-110 active:translate-y-1 hover:bg-white/10 shadow-lg outline-none transform-gpu ${isActive ? 'border-neon-amber shadow-[0_0_15px_rgba(255,191,0,0.2)]' : 'border-grey-medium'}`}
    >
      ...
      <span className={`absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-mono text-neon-amber opacity-0 transition-opacity uppercase whitespace-nowrap ${isActive ? '' : 'group-hover/paper:opacity-100 group-focus-visible/paper:opacity-100'}`}>
        {title}
      </span>
      <FocusIndicator color="neon-amber" group="paper" />
    </button>
  );
};
...
export const Papers: React.FC<PapersProps> = ({ context = 'user', labelPosition = 'top' }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2 relative">
      <div className={`absolute left-1/2 -translate-x-1/2 h-4 flex items-center justify-center whitespace-nowrap ${labelPosition === 'top' ? '-top-8' : '-bottom-8'}`} aria-live="polite">
        {activeTitle && (
