...
const Paper: React.FC<PaperProps> = ({ label, title, rotation = '0deg', translateY = '0px', isActive, labelPosition = 'top', onClick }) => {
  ...
  return (
    <button
      ...
      className={`group relative w-12 h-16 bg-white/5 border transition-all hover:rotate-0 hover:translate-y-0 hover:scale-110 active:translate-y-1 hover:bg-white/10 shadow-lg outline-none transform-gpu ${isActive ? 'border-neon-amber shadow-[0_0_15px_rgba(255,191,0,0.4)]' : 'border-grey-medium'}`}
    >
...
export const Papers: React.FC<PapersProps> = ({ context = 'user', labelPosition }) => {
  ...
  // Default label position based on context if not explicitly provided
  const effectiveLabelPosition = labelPosition || (context === 'user' ? 'top' : 'bottom');
