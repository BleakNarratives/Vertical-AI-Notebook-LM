...
export const CoffeeMug: React.FC = () => {
  ...
  const handleAction = useCallback(() => {
    ...
    timeoutRef.current = setTimeout(() => { setStatus(null); timeoutRef.current = null; }, 2000);
  }, []);
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      >
        ...
        <div className={`absolute -top-6 left-4 flex gap-1.5 transition-all duration-500 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100'}`}>
          <div className={`w-0.5 h-4 animate-[bounce_2s_infinite] blur-[1px] ${isActive ? 'bg-neon-amber' : 'bg-grey-medium/40'}`} />
          <div className={`w-0.5 h-6 animate-[bounce_1.5s_infinite] blur-[1px] ${isActive ? 'bg-neon-amber shadow-[0_0_8px_rgba(255,191,0,0.8)]' : 'bg-grey-medium/40'}`} />
        </div>
