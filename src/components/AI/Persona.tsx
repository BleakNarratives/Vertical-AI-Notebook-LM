...
export const Persona: React.FC<PersonaProps> = ({ name, role, status, onClick, disabled }) => {
  return (
    ...
    >
      ...
      <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-center whitespace-nowrap transition-opacity duration-300 pointer-events-none">
        <h3 className="text-xs font-mono text-neon-red uppercase tracking-[0.2em]">{name}</h3>
        <p className="text-xs font-mono text-white/40 uppercase opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100">{role}</p>
      </div>

      <FocusIndicator variant="red" />
    </button>
  );
};
