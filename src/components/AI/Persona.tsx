...
export const Persona: React.FC<PersonaProps> = ({ name, role, status, onClick, disabled }) => {
  return (
    ...
    >
      {/* Floor Glow */}
      <div className={`absolute -inset-4 bg-radial-gradient(circle,rgba(255,0,0,0.15)_0%,transparent_70%) opacity-0 transition-opacity duration-500 pointer-events-none ${status === 'active' ? 'opacity-100' : ''}`}
      ...
      <div className="text-center">
        <h3 className="absolute -top-8 left-1/2 -translate-x-1/2 text-xs font-mono text-neon-red uppercase tracking-[0.2em] whitespace-nowrap">{name}</h3>
